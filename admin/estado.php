...
<body>
  ...
  <h1 style="text-align:center;" class="my-3">Estado</h1>
  ...
  if(isset($_GET['id'])) {
    $id = $_GET['id'];
    foreach($pdo->selectEstadoCidade($id) as $estado) {
      // var_dump($estado);
    
      echo "<tr>
        <td>{$estado['id']}</td>
        <td><a href=".'cidade.php?id_estado='.$estado['id'].">Cidade</a></td>
        <td>{$estado['estado']}</td>".
        '<td>
              <a href="update/updateestado.php?id='.$estado['id'].'">
                <i class="bi bi-pencil text-primary"></i>
              </a>
            </td>
            <td>
              <a href="deletar.php?id='.$estado['id'].'&tabela=estado">
    ...
    foreach($pdo->selectEstado() as $estado) {
      // var_dump($estado);
