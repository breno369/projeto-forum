...
<body>
  ...
  <h1 style="text-align:center;" class="my-3">Cidade</h1>
  ...
  if(isset($_GET['id'])) {
    $id = $_GET['id'];
    foreach($pdo->selectCidadeCep($id) as $cidade) {
      // var_dump($cidade);
  
      echo "<tr>
      <td>{$cidade['id']}</td>
      <td><a href=".'estado.php?id='.$cidade['id_estado'].">Estado</a></td>
        <td>{$cidade['cidade']}</td>".
        '<td>
              <a href="update/updatecidade.php?id='.$cidade['id'].'">
                <i class="bi bi-pencil text-primary"></i>
              </a>
            </td>
            <td>
              <a href="deletar.php?id='.$cidade['id'].'&tabela=cidade">
    ...
    $id = $_GET['id_estado'];
    foreach($pdo->selectIdEstado($id) as $cidade) {
    ...
    foreach($pdo->selectCidade() as $cidade) {
      // var_dump($idestado);
      // var_dump($cidade);
